...
class ProfileView extends React.Component {
  constructor() {
    ...
    this.get_viewee_data_from_server =
      this.get_viewee_data_from_server.bind(this);
  ...
  get_viewee_data_from_server() {
    let viewee_username = new URLSearchParams(window.location.search).get(
      "viewee_username"
    );
  ...
  sort_posts_by_date = (posts) => {
    let cmp_function = (post1, post2) => {
      return post2.post.date - post1.post.date;
    };
    ...
    return (
      <div>
        ...
        >
          ...
          >
            {this.sort_posts_by_date(this.state.items).map(
              (post_view, index) => (
                <Post
