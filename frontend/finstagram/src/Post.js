...
export default function Post(props) {
  ...
  const [follow, setFollow] = React.useState(
    props.viewer.followees.includes(props.username) ||
  ...
  return (
    <Card sx={{ maxWidth: 445 }} style={{ paddingTop: "0%" }}>
      <CardHeader
        ...
        avatar={
          <a
            href={`/profile_view?viewer_username=${props.viewer.username}&viewee_username=${props.username}`}
          >
            {" "}
          ...
          <div sx={{ height: "20%" }}>
            ...
            {!follow ? (
              <Button
                variant="outlined"
                sx={{ height: "20px" }}
                style={{ fontSize: "2.0vh", marginLeft: "2%" }}
      ...
      <CardActions disableSpacing>
        ...
        {props.viewer.username == props.username ? (
          <IconButton aria-label="delete">
            <DeleteIcon
              onClick={() => {
                props.delete_post(props.post_id);
              }}
            />
          </IconButton>
        ) : (
      ...
      <Collapse in={expanded} timeout="auto" unmountOnExit>
        <CardContent sx={{ width: "100%", padding: "0 0" }}>
          <CommentSet
            ...
            add_comment_to_database={add_comment_to_database}
          />
        </CardContent>
      </Collapse>
    </Card>
